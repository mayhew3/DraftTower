...
class Lineup {
  ...
  function getLineupArray() {
    $lineupArray = array();
    $orderedPositions = array("C", "1B", "2B", "3B", "SS", "OF", "DH", "P", "RS");
    foreach($orderedPositions as $position) {
      foreach ($this->getPlayersInPosition($position) as $player) {
        $lineupArray[] = array($position, $player);
      }
      for ($i = 0; $i < $this->getOpeningsForPosition($position); $i++) {
        $lineupArray[] = array($position, null);
      }
    }
    return $lineupArray;
  }
